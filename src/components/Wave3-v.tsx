...
function Wave3v({ srcAudio, srcCanvas, options, audioContext }: VizComponentProps) {
  ...
  if (options && Array.isArray(options)) {
    userOptions = {
      color: [options[0]], // user-passed color
    };
  }

  const defaults = {
    domain: ['time', 2048],           // smoother wave
    style: ['line'],                  // line style
    color: ['#00ffff'],               // bright neon cyan
    glow: [true],                     // glow on
    amplitudeScale: [3.5],           // strong amplitude
    stroke: [4],                      // thick lines
  };
