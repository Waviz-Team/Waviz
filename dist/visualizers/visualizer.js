...
const mathUtils_2 = require("../utils/mathUtils");
// Visualizer class
class Visualizer {
    constructor(canvas, data) {
        ...
        if (!data || !(data instanceof analyzer_1.default)) {
    ...
    dataPreProcessor(dataType = 'time', amplitude = 100, range = 1024, windowName //? Q mark added here since windowName shouldn't be in freq
        ...
        if (dataType === 'time' && windowName) {
            switch (windowName.toLowerCase()) {
                case 'hanning':
                case 'hann':
                    processedData = mathUtils_1.windowFunc.hann(processedData);
                    break;
                case 'hamming':
                    processedData = mathUtils_1.windowFunc.hamming(processedData);
                    break;
                case 'exponential':
                case 'exp':
                    processedData = mathUtils_1.windowFunc.exponential(processedData);
                    break;
                case 'blackman-harris':
                case 'bharris':
                case 'blackmanh':
                    processedData = mathUtils_1.windowFunc.bHarris(processedData);
                    break;
                default:
                    break;
            }
    ...
    dataToPolar(input, radius = 100, angle = 0, autoRotate = 0) {
        ...
        const polarData = [];
        const periodicData = (0, mathUtils_2.makePeriodic)(input); // this can be moved into an option later if needed. Right now, forces linear tilt on polar to smooth out end/beginning seam
