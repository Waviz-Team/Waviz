...
function Mixed7({ srcAudio, srcCanvas, options, audioContext, }) {
    ...
    const [canvasReady, setCanvasReady] = (0, react_1.useState)(false); // Needed in case of defaulting back to preset canvas. UseRef only will not trigger page re-render, causing visualizer to run before canvas is rendeteal
    // User options
    let userOptions = {};
    if (options) {
        userOptions = [
